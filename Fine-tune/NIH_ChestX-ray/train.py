...
def valid(args, model, writer, test_loader, global_step):
    ...
    # print(all_property.size())
    # print('label',type_of_target(all_label))
    # print('all_property',type_of_target(all_property))
    # pdb.set_trace()
...
def test(args, model, test_loader):
    ...
    label_path = "./result/"+ args.name+ "/" + args.pretrained_dir.split('/')[-1].split('.')[0]
    if not os.path.exists(label_path):
        os.makedirs(label_path)
    for i in range(len(CLASS_NAMES)):
        file_name = os.path.join(label_path ,CLASS_NAMES[i] + '.txt')
    ...
    with open(label_path+ "/result.txt","w",encoding="utf-8") as fw:
